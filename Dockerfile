...
# disabled-password → can't log in using a password
#--home "/nonexistent" → gives it a home path that doesn't actually exist
#--shell "/sbin/nologin" → prevents normal interactive login
#--no-create-home → don't create a home directory
#--uid "${UID}" → assign UID 10001
# GEcos stands for General Electric Comprehensive Operating System. In Linux, though, you can think of it simply as user information/comment fields
#ARG UID=10001
RUN adduser \
    --disabled-password \ 
    --gecos "" \
